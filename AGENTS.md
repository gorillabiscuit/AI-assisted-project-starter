...
This file is the contract between the human maintainer and the AI coding agent on this repo — whichever harness the agent runs in (Claude Code, Pi, or any other AGENTS-compatible tool). It exists because AI-assisted code accumulates invisible technical debt fast without explicit guardrails. Every rule has been chosen because it catches an *AI-specific* failure mode, not just as general good engineering.
...
Each `apps/*` and `packages/*` has its own `AGENTS.md` for package-specific rules (with a `CLAUDE.md` symlink alongside). Root `AGENTS.md` (this file) holds global rules.
...
- **North Star relevance gate.** Before starting any non-trivial task — or whenever scope expands past the original ask — state in one line how the work traces to `PROJECT.md`'s North Star. If it can't be traced, stop and surface it instead of proceeding. **The North Star takes precedence:** when a request and the North Star conflict, flag the conflict and let the human resolve it rather than silently doing both. If `PROJECT.md`'s North Star block is missing, still placeholder text (`<...>` markers, "fill on kickoff"), or hasn't been confirmed by running the kickoff ritual (the `/north-star` skill — install per `docs/north-star-kickoff.md`), treat that itself as a stop-and-ask — run the ritual in greenfield or retrofit mode (per Phase 0 of the skill) before any feature work.
  **Which parts of this gate are machinery vs prose:** (a) the *unfilled-star precondition* is hook-enforced — a `PreToolUse` hook (`.claude/settings.json` → `scripts/hooks/block-unstarred-source-edit.sh`) blocks edits under `apps/` and `packages/` while the block is missing or placeholder; (b) the star is *injected into context* every session by a `SessionStart` hook (`scripts/hooks/inject-north-star-session-start.sh`), so "it wasn't in frame" is off the table; (c) the *trace scoring* happens at pre-PR — the `pessimistic-reviewer` subagent classifies every branch commit TRACES / TENUOUS / DOES-NOT-TRACE cold (§7). Only the per-task one-line trace *during* the session remains prose — state it anyway; the reviewer scores whether it would have survived.
  **In this starter repo itself:** `PROJECT.md`'s block stays placeholder by design (it is the shipped template that arms the gate in fresh clones); the starter's own North Star lives in `docs/NORTH-STAR.md` and work on this repo traces against that file.
...
Weigh proposed conventions against the North Star: a rule that won't ever influence the metric in §4's gate is probably not worth the line of overhead. Per §4, the North Star takes precedence when a proposed rule and the North Star conflict.
