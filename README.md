...
- North Star enforcement hooks — the relevance gate is machinery, not
  ...
  injects the star into context every session — the Metric framed in a
  bordered box so the one line every task traces to is impossible to
  miss (the full ASCII star scene is the `/north-star` ritual's
  finalisation reveal, not re-flashed every session) — and the `/pre-pr`
